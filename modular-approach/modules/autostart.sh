...
setup_autostart() {
    ...
    cat > "$HOME/.local/bin/ubuntu-unity-startup" << 'EOF'
#!/bin/bash
# Ubuntu Unity Startup Script
...
echo "Ubuntu Unity startup at $(date)" >> "$LOG_FILE"
...
log_startup "Ubuntu Unity startup complete"
EOF
    
    chmod +x "$HOME/.local/bin/ubuntu-unity-startup"
    log "✓ Startup script created"
    
    # Create autostart desktop entry
    log "Creating autostart desktop entry..."
    cat > "$HOME/.config/autostart/ubuntu-unity-enhancements.desktop" << 'EOF'
[Desktop Entry]
Type=Application
Name=Ubuntu Unity Enhancements
Comment=Start Ubuntu Unity enhancements (hotkeys, MMB, etc.)
Exec=/home/ian/.local/bin/ubuntu-unity-startup
Hidden=false
NoDisplay=false
...
EOF
    ...
    sed -i "s|/home/ian|$HOME|g" "$HOME/.config/autostart/ubuntu-unity-enhancements.desktop"
    ...
    cat > "$HOME/.config/systemd/user/ubuntu-unity.service" << 'EOF'
[Unit]
Description=Ubuntu Unity Enhancements
After=graphical-session.target
Wants=graphical-session.target

[Service]
Type=forking
ExecStart=%h/.local/bin/ubuntu-unity-startup
Restart=on-failure
RestartSec=5
...
EOF
    ...
    systemctl --user enable ubuntu-unity.service
    
    log "✓ Systemd service created and enabled"
    
    log "Autostart setup complete"
    log "- Services will start automatically on login"
    log "- Manual restart: ~/.local/bin/ubuntu-unity-startup"
}
...
test_autostart() {
    ...
    if [[ -f "$HOME/.config/autostart/ubuntu-unity-enhancements.desktop" ]]; then
    ...
    if systemctl --user is-enabled ubuntu-unity.service >/dev/null 2>&1; then
